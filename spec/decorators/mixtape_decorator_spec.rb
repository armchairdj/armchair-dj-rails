...
RSpec.describe MixtapeDecorator do
  let( :playlist) { create(:minimal_playlist, title: "This Is the Title") }
  let(:decorator) { MixtapeDecorator.new(mixtape) }

  describe "#title" do
    subject { decorator.title(opts) }

    let(:mixtape) { create(:minimal_mixtape, playlist: playlist) }
