...
RSpec.describe VideoGame, type: :model do
  ...
  context "STI" do
    context "class" do
      ...
      specify { expect(described_class.true_human_model_name).to eq("Video Game") }
    end

    context "instance" do
      let(:instance) { create_minimal_instance }

      specify { expect(instance.type                 ).to eq("VideoGame") }
      specify { expect(instance.true_model_name.name ).to eq("VideoGame") }
      specify { expect(instance.true_human_model_name).to eq("Video Game") }
    end
  end
