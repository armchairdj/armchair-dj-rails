...
RSpec.describe Review, type: :model do
  ...
  context "class" do
    describe "self#publish_scheduled" do
      let!(:current) { create(:minimal_post, :scheduled, publish_on: 1.day.from_now) }
      let!( :future) { create(:minimal_post, :scheduled) }
  ...
  context "scope-related" do
    let!(:draft_standalone    ) { create(:minimal_post, :draft    ) }
    let!(:scheduled_standalone) { create(:minimal_post, :scheduled) }
    let!(:published_standalone) { create(:minimal_post, :published) }
    let!(:published_review    ) { create(:minimal_review,          :published) }
    let!(:scheduled_review    ) { create(:minimal_review,          :scheduled) }
    let!(:draft_review        ) { create(:minimal_review,          :draft    ) }
  ...
  context "aasm" do
    let!(    :draft) { create(:minimal_post, :with_body, :draft    ) }
    let!(:scheduled) { create(:minimal_post, :with_body, :scheduled) }
    let!(:published) { create(:minimal_post, :with_body, :published) }
    ...
    describe "private" do
      context "guards" do
        describe "#ready_to_publish?" do
          subject { build(:minimal_post, :with_body) }
      ...
      context "callbacks" do
        ...
        describe "#clear_published_at" do
          it "removes published_at" do
            instance = create(:minimal_post, :published)
    ...
    describe "update-transition methods" do
      describe "#update_and_publish" do
        subject { create(:minimal_post, :with_body) }
  ...
  context "instance" do
    context "decorators" do
      let(:standalone) { create(:minimal_post) }
    ...
    context "booleans" do
      context "for type" do
        let(:unsaved_standalone) {  build(:minimal_post) }
        let(  :saved_standalone) { create(:minimal_post) }
        let(    :unsaved_review) {  build(:minimal_review) }
        let(      :saved_review) { create(:minimal_review) }
    ...
    describe "#sluggable_parts" do
      ...
      let(:standalone) { create(:minimal_post, title: "Standalone Title") }
